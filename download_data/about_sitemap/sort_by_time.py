...
# default
ERROR_URL_SAVE_PATH = "./error_urls.txt"
...
def parse_six_lang_pdf_links(six_lang_pdf_links):
    ...
    soup = BeautifulSoup(file_selection_page_html_text)
...
if __name__ == '__main__':
    ...
    with open(f"{args.downloaded_pdf_url_dir}/SixLanguagePDF-URLS.json", "w") as f:
    ...
    if worker_thread == 1:
        for record_list_row in tqdm(parsed_dict_list):
    ...
    with open(f"{args.file_save_path}/SixLanguageURL-Information.json", "w") as f:
        json.dump(parsed_dict_list)