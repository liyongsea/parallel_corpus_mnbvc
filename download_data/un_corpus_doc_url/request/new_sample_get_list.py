...
session.headers = {
    ...
    "Host":"search.un.org",
    "Pragma":"no-cache",
    "Referer":"https://search.un.org/search?sort=relevance&collection=ods&currentPageNumber=1&fromYear=2018&q=*&row=100&toYear=2018",
...
while i < mLen:
    if not is_cache_exists(i):
        nexturl = f'https://search.un.org/api/search?collection=ods&currentPageNumber={i}&fromYear={FROM_YEAR}&q=*&row=100&sort=relevance&toYear={TO_YEAR}&mLen={mLen}&td={td}'
