...
TARGET_LANG = 'en'
TEMPDIR_TRANSLATION = Path(r'F:\undl_text_translation_temp') # 中间翻译暂存，支持重启
OUTPUT_DIR_TRANSLATION = Path(r'F:\undl_text_translation') # datasets输出
...
def main():
    # {ar: str, zh: str, en: str, fr: str, ru: str, es: str, de: str, record: str}
    dataset = datasets.load_from_disk(r'F:\undl_text_local') # 形如 https://huggingface.co/datasets/bot-yaya/undl_text 的输入
    ...
    for src_lang in ALL_SOURCE_LANGS:
        ...
        for idx, row in enumerate(dataset):
            dump_path = src_lang_dump / f"{idx}.pkl" # 每条记录存单文件，如果需要改并发就可以这么改
            if dump_path.exists():
                continue
            src_text = list(filter(bool, (clean_paragraph(x) for x in re.split('\n\n', row[src_lang])))) # \n\n分段，然后每段清理噪声文本，然后滤掉空段
            print(f"{idx} {src_lang} -> {TARGET_LANG}, paragraph count={len(src_text)}")
            begin = datetime.now()
        ...
        def dataset_generator():
            for idx, row in enumerate(dataset):
                dump_path = src_lang_dump / f"{idx}.pkl"
